 ...
 *
 */
@Service("chatEachOtherContentValidator")
public class ChatEachOtherContentValidator implements MessageContentValidator<Replies> {
	private static final Logger logger = LoggerFactory.getLogger(ChatEachOtherContentValidator.class);
	@Override
	public String name() {
		return ReplyTypeEnum.RESP.name();
	}
	@Override
	public void validate(Replies content) throws Exception {
		...
		if (StringUtils.isBlank(content.getXxlx()) || !name().equals(content.getXxlx())) {
			throw new ServiceException("回复内容的类型不合法或者不是针对回复的回复!");
		}
		if (StringUtils.isBlank(content.getReferenceId())) {
			throw new ServiceException("缺少回复id.");
		}
		...
		if (StringUtils.isBlank(content.getHfry())) {
			throw new ServiceException("缺少回复接收人编号");
		}
		if (StringUtils.isBlank(content.getFsnr())) {
			throw new ServiceException("没有反馈内容!");
		}
		logger.debug("chat from User[" + content.getFsdw() + "," + content.getFsry() + "] to User[" + content.getHfry()
				+ "," + content.getHfdw() + "] on RESP[" + content.getReferenceId() + "]");
	}
