...
public class LocalWebSocketFrameResolver extends WebSocketFrameResolver implements WebSocketMessageReceiver {
	private static final Logger logger = LoggerFactory.getLogger(LocalWebSocketFrameResolver.class);

	public LocalWebSocketFrameResolver() {
	...
	protected void onRequest(WebSocketContext context) {
		...
		if (requestParameters == null) {
			error.append("request parameter['ajbh','account'] were missed.");
		} else {
			/*List<String> ajbh = requestParameters.get("ajbh");// 案件编号
			if (CollectionUtils.isEmpty(ajbh)) {
				logger.warn("request parameter['ajbh'] was missed or it's value was illegal.");
			} else {
				if (StringUtils.isBlank(ajbh.get(0))) {
					logger.warn("request parameter['ajbh'] was missed or it's value was illegal.");
				}
			}*/
		...
		if (error.length() > 0) {
			...
			//String ajbh = requestParameters.get("ajbh").get(0);
			String account = requestParameters.get("account").get(0);
			//logger.info("ajbh===>" + ajbh + ",login account====>" + account);
			logger.info("login account====>" + account);
			//context.setChannelAttribute("ajbh", ajbh);
	...
	@Override
	protected void onMessage(String msg, WebSocketContext context) {
	...
	public void onWebSocketMessageReceived(WebSocketMessage instance) {
		...
		if (instance != null && context != null) {
			if (CommunicationChannels.LOCAL.equals(instance.getFrom())) {
				String id = context.getId();
				logger.debug(CommunicationChannels.LOCAL+" Context Id " + id);
				Set<String> toUserIdSet = instance.getTo();
				if (instance.getBody() != null && CollectionUtils.isNotEmpty(toUserIdSet)) {
					String fromUserId = context.getChannelAttribute(WebSocketResolverConstants.CONTEXT_USER);
					if (StringUtils.isNotBlank(fromUserId)) {
						toUserIdSet.add(fromUserId);
					}
